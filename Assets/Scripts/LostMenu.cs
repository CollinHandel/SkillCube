...
public class LostMenu : MonoBehaviour {
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(Screen.width / 4, Screen.height / 8, Screen.width / 2, Screen.height / 4), "You Lost");

        if (GUI.Button(new Rect(Screen.width / 2.25f, Screen.height / 2.5f, Screen.width / 8, Screen.height / 12), "Retry"))
        {
            SceneManager.LoadScene("Level " + PlayerPrefs.GetInt("Level Completed"));
        }
        
        if (GUI.Button(new Rect(Screen.width / 2.25f, Screen.height / 2.0f, Screen.width / 8, Screen.height / 12), "Menu"))
        {
